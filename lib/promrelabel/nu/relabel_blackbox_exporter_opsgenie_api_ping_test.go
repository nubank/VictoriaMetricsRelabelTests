...
func TestRelabelBlackboxExporterOpsgenieApiPing(t *testing.T) {
  ...
  `{__address__="https://api.opsgenie.com/ping",__metrics_path__="/probe",__param_module="opsgenie_api_ping",__scheme__="http",__scrape_interval__="30s",__scrape_timeout__="30s",job="blackbox-exporter-opsgenie-api-ping"}`,

  true,

  `{endpoint="https://api.opsgenie.com/ping",job="blackbox-exporter-opsgenie-api-ping"}`)
}