...
func TestRelabelConfluentResources(t *testing.T) {
  ...
  `{__address__="localhost:7778",__metrics_path__="/metrics",__scheme__="http",__scrape_interval__="2m",__scrape_timeout__="2m",job="confluent-resources"}`,

  true,

  `{job="confluent-resources"}`)
}